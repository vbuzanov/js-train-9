...
function findIndexByProperty(arr, name) {
  ...
  if (Array.isArray(arr)) {
    const foundIndex = arr.findIndex((elem) => elem.name === name);
    return foundIndex;
  } else {
    return -1;
  }
...
function getPositiveArrayValues(arr) {
  ...
  if (Array.isArray(arr)) {
    ...
    const positiveArrayValues = arrayValues.filter((item) => item > 0);
...
function getLastElementsSumAndPrint(arr, n) {
  ...
  if (Array.isArray(arr)) {
    const arr1 = arr.slice(arr.length - n);
    const sum = arr1.reduceRight(
      (accumulator, currentValue) => accumulator + currentValue
    );
    // const sum = arr.reduceRight(
    //   (accumulator, currentValue, currentIndex, array) => {
    //     if (currentIndex >= array.length - n) {
...
function expandArrayByN(arr, n) {
  ...
  if (Array.isArray(arr) && typeof n == "number") {
    const expandedArray = arr.flatMap((elem) => Array(n).fill(elem, 0, n));
...
function findDuplicateElements(arr) {
  ...
  if (Array.isArray(arr)) {
    let duplicateElements = arr.filter((elem, index, array) => {
      return (
...
function capitalizeWords(arr) {
  ...
  if (Array.isArray(arr)) {
    const capitalizedArray = arr.map((elem) => {
